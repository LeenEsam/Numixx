  ...
  try {
    const { data, error } = await supabase
      ...
      .select(`
        ...
        profiles(full_name)
      `)
      .eq("lesson_id", lessonId); // 🔥 فلترة على الدرس الحالي فقط

    if (error) throw error;

    // 🔹 تجميع البيانات حسب الرابط
    const grouped = {};
    data.forEach((item) => {
...
useEffect(() => {
  ...
  const handleFileUpload = async (file, type) => {
    ...
    const fileUrl = `https://qkghyltnktnadcxdigqn.supabase.co/storage/v1/object/public/lesson-files/${encodeURIComponent(fileName)}`;

    const field = type === "pdf" ? "pdf_urls" : "video_urls";

    // اجلب البيانات الحالية
